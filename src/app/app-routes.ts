    ...
    children: [
      ...
      },
      {
        ...
        loadChildren: () => import('./forgot-password/forgot-password-routes')
          .then((m) => m.ROUTES),
        // Removed forgotPasswordCheckGuard - it redirects unauthenticated users to login
        // which breaks the forgot password flow
      },
      {
      ...
      // Backwards-compat: redirect legacy /help URLs to current /info routes
      { path: 'help', pathMatch: 'full', redirectTo: 'info' },
      { path: 'help/:topic', pathMatch: 'full', redirectTo: 'info/:topic' },
